...
uses UnDM, UnMain, Unproperedit, Unglobal;

procedure TFormproper.BtnaddClick(Sender: TObject);
begin
...
//    DM.ADO_URGP_query.Edit;
    ...
    if save=1 then
      RecordLog('添加特征码:'+DM.ADO_PROP_query.FieldByName('PROPER').AsString);
    Formproperedit.Free;
    Formproperedit:=nil;
...
procedure TFormproper.BtncloseClick(Sender: TObject);
begin
//  Btnsave.Click;
  Formproper.close;
end;

procedure TFormproper.BtndelClick(Sender: TObject);
begin
//
    ...
    begin
      if Application.MessageBox('确定删除选定特征码？ ','确认',MB_YESNO) = 6 then
        begin
          ...
          RecordLog('删除特征码:'+DM.ADO_PROP_query.FieldByName('PROPER').AsString);
...
procedure TFormproper.BtneditClick(Sender: TObject);
begin
...
//    DM.ADO_URGP_query.Edit;
    //freeandnil(Formshowcol);
    if save=1 then
      RecordLog('编辑特征码:'+DM.ADO_PROP_query.FieldByName('PROPER').AsString);
    Formproperedit.Free;
    Formproperedit:=nil;
END;
end;

procedure TFormproper.cxGrid1DBTableView1FocusedItemChanged(
  Sender: TcxCustomGridTableView; APrevFocusedItem,
  AFocusedItem: TcxCustomGridTableItem);
begin
//  DM.ADO_ACTI_query.Close;
...
procedure TFormproper.cxGrid1DBTableView1FocusedRecordChanged(
  Sender: TcxCustomGridTableView; APrevFocusedRecord,
...
procedure TFormproper.FormClose(Sender: TObject; var Action: TCloseAction);
begin
   Formproper.Free;
   Formproper:=nil;
end;

procedure TFormproper.FormCreate(Sender: TObject);
begin
//  DateTimePicker1.Date:=now();
//  DateTimePicker2.Date:=now();
end;

procedure TFormproper.FormShow(Sender: TObject);
begin
